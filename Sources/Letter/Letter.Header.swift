...
extension Letter.Header: HTML {
    @HTMLBuilder public var body: some HTML {
        table {
            tr {
                td { recipient }.inlineStyle("vertical-align", "top").inlineStyle("width", "100%")

                td { sender }.inlineStyle("vertical-align", "top")
            }

        }.inlineStyle("width", "100%").inlineStyle("border-collapse", "collapse")

        switch (location, date.sending) {
        case (.some(let location), .some(let date)):
            HTMLText(
                "\(location), \(date.formatted(date: .long, time: .omitted, translated: true))"
            )
            br()
        ...
        case (.none, .some(let date)):
            ...
            br()
        case (.none, .none): HTMLEmpty()
        }

        if let subject {
            TranslatedString(dutch: "betreft", english: "subject").capitalizingFirstLetter()
            ": "
            "\(subject)"
            br()
        }
...
extension Letter.Header {
    ...
    import SwiftUI
    #Preview { HTMLDocument { Letter.Header.preview }.frame(width: 451, height: 698) }
#endif
