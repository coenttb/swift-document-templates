...
extension Letter {
    public struct Recipient: Hashable, Equatable, Codable {
        ...
        public init(name: String, address: [String], emails: [String]) {

            var metadata: Metadata = [:]
            metadata[.perEmail] = emails.joined(separator: ", and")

            self = .init(name: name, address: address, metadata: metadata)
        }
    }
}

extension Letter.Recipient {
    public static func mock() -> Self { .init(name: "Mock", address: []) }
