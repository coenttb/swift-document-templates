...
extension Signatory {
    ...
    public var name: TranslatedString {
        ...
        case .individual(let name, _): return name
        }
    }
    ...
    public var people: [Person] {
        switch self {
        case .group(_, let people, _): return people
    ...
    public var metadata: Metadata {
        switch self {
        case .group(_, _, let metadata): return metadata
        case .individual(_, let metadata): return metadata
        }
    }

    /// Convenience initializer for a signatory group
    public init(name: TranslatedString, signers: [Person], metadata: Metadata = [:]) {
        self = .group(name: name, signers: signers, metadata: metadata)
    }

    /// Convenience initializer for a signatory group with a single Person
    public init(name: TranslatedString, signer: Person, metadata: Metadata = [:]) {
    ...
    public struct Person: Hashable, Codable {
        ...
        public init(name: TranslatedString, metadata: Metadata = [:]) {
            self.name = name
        ...
        public struct Block: HTML {
            ...
            public init(
                ...
                if let date {
                    combinedMetadata[.date.capitalizingFirstLetter()] = .init(
                        date.formatted(date: .numeric, time: .omitted)
                    )
                }
                if let location {
                    combinedMetadata[.location.capitalizingFirstLetter()] = TranslatedString(
                        location
                    )
                }
                    ...
                    if !person.metadata.isEmpty || !metadata.isEmpty {
                        table {
                            ...
                            HTMLForEach(person.metadata.map { $0 }) { key, value in
                                tr {
                                    td { key.map { $0.capitalizingFirstLetter() } }.width(
                                        .px(.init(style.metadataColumnWidth))
                                    ).padding(right: .px(15)).verticalAlign(.top)

                                    td { value }.verticalAlign(.top)
                                }
                            }
                            ...
                            HTMLForEach(metadata.map { $0 }) { key, value in
                                tr {
                                    td { key }.width(.px(.init(style.metadataColumnWidth))).padding(
                                        right: .px(15)
                                    ).verticalAlign(.top)
                        ...
                        }.borderCollapse(.collapse).padding(top: .px(5))
                    }

                    // Signature line
                    div { String(repeating: "_", count: 40) }.padding(
                        vertical: .px(30),
                        horizontal: nil
                    ).maxWidth(.px(300))
                }.margin(bottom: .px(15))
...
extension Signatory {
    static var preview: Self {
        .group(
            name: TranslatedString(dutch: "Acme B.V.", english: "Acme Corporation"),
            signers: [
                Person(name: "John Doe", position: "CEO"),
                Person(name: "Jane Smith", position: "CFO"),
            ],
            metadata: [.registrationNumber: "12345678", .role: .seller]
        )
    }

    static var individualExample: Self {
        .individual(name: "Coen ten Thije Boonkkamp", position: "Developer", metadata: [:])
    }
}
