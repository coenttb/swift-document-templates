...
public struct AttendanceList: Sendable {
    ...
    public init(title: String, metadata: Metadata = [:], attendees: [AttendanceList.Attendee]) {
        self.title = title
...
extension AttendanceList: HTML {
    public var body: some HTML {
        h1 { TranslatedString(dutch: "Aanwezigheidslijst", english: "Attendance List") }

        table {
            HTMLForEach(self.metadata.map { $0 }) { key, value in
                tr {
                    td { "\(key)" }.inlineStyle("text-align", "right").inlineStyle(
                        "vertical-align",
                        "top"
        ...
        table {
            tr {
                th { HTMLText("Last name") }.inlineStyle("padding-block", "15px")
                th { HTMLText("First name") }.inlineStyle("padding-block", "15px")
                th { HTMLText("Role") }.inlineStyle("padding-block", "15px")
                th { HTMLText("Signature") }.inlineStyle("padding-block", "15px")
            }.inlineStyle("border", "1px solid black")

            HTMLForEach(attendees) { attendee in
                HTMLGroup {
                    tr {
                        td { HTMLText(attendee.lastName) }.inlineStyle(
                            "padding-block",
                            "\(!attendee.lastName.isEmpty ? "15px" : "25px")"
                        )
                        td { HTMLText(attendee.firstName) }.inlineStyle(
                            "padding-block",
                            "\(!attendee.firstName.isEmpty ? "15px" : "25px")"
                        )
                        td { HTMLText(attendee.role) }.inlineStyle(
                            "padding-block",
                            "\(!attendee.role.isEmpty ? "15px" : "25px")"
                        )
        ...
        }.width(.percent(100)).borderCollapse(.collapse)
