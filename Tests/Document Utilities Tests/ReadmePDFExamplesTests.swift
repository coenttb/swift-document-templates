...
// These tests are primarily for documentation and work best on macOS
    ...
    @Suite("README PDF Examples") struct ReadmePDFExamplesTests {
        ...
        /// Output directory for generated PDFs (.github/Images/ for README)
        private static let outputDirectory: URL = {
            // Navigate to repository root and use .github/Images/ directory
            let testFile = URL(fileURLWithPath: #filePath)
            let testsDir = testFile.deletingLastPathComponent().deletingLastPathComponent()
            let repoRoot = testsDir.deletingLastPathComponent()
            return repoRoot.appendingPathComponent(".github", isDirectory: true)
