...
struct ReadmeVerificationTests {
    ...
    @Test("Invoice example from README (lines 51-84)")
    func invoiceExample() async throws {
    ...
    @Test("Invoice can render as HTML")
    func invoiceRendersAsHTML() async throws {
        try await withDependencies {
            $0.calendar = .autoupdatingCurrent
        } operation: {
            let invoice = Invoice(
    ...
    @Test("DateExtensions .days syntax works")
    func dateExtensionsDays() async throws {
        try await withDependencies {
            $0.calendar = .autoupdatingCurrent
        } operation: {
            let now = Date.now
